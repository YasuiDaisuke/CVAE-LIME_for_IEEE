...
def AE_training(X_train=None,
    ...
    if auto_encoder == 'CVAE':
        ...
        num_classes = int(np.max(y_train) + 1)  # クラス数を取得
        y_train_onehot = to_categorical(y_train, num_classes=num_classes)
        y_valid_onehot = to_categorical(y_valid, num_classes=num_classes)
        y_test_onehot = to_categorical(y_test, num_classes=num_classes)  # テストデータも同様に
        
        # 2. condition_dimをone-hotエンコーディングされたベクトルの長さに更新
        condition_dim = y_train_onehot.shape[1]
        ...
        history = vae.fit([X_train, y_train_onehot], None, shuffle=True, epochs=epochs, batch_size=16, validation_data=([X_valid, y_valid_onehot], None))
        ...
        X_test_reconstructed = vae.predict([X_test, y_test_onehot])
        reconstruction_mse = np.mean(np.power(X_test - X_test_reconstructed, 2), axis=1)
        print('Reconstruction error:', np.mean(reconstruction_mse))
...
def AE_load(X_test=None,
    ...
    if auto_encoder_weighting == True and auto_encoder_sampling == False:
        ...
        # latent_vectorとlatent_vectorsがAE以外ではリスト形式になるので，np.arrayに統一
