...
def AE_training(X_train=None,
    ...
    class CustomModelCheckpoint(Callback):
        def __init__(self, encoder, decoder, latent_dim, condition_dim, auto_encoder, dataset, save_interval):
        ...
        def on_epoch_end(self, epoch, logs=None):
            if (epoch + 1) % self.save_interval == 0:
                # エンコーダを保存
                self.encoder.save(f'save_data/auto_encoder_model/model/{self.auto_encoder}_{self.dataset}_encoder_dim{self.latent_dim}_onehot{one_hot_encoding}_Dense{Dense_dim}_epoch{epoch+1}.h5')
                
                # デコーダを保存
                decoder_input_dim = self.latent_dim + self.condition_dim
                decoder_input = Input(shape=(decoder_input_dim,))
                _h_decoded = self.decoder(decoder_input)
                _x_decoded_mean = decoder_mean(_h_decoded)
                decoder = Model(decoder_input, _x_decoded_mean)
                decoder.save(f'save_data/auto_encoder_model/model/{self.auto_encoder}_{self.dataset}_decoder_dim{self.latent_dim}onehot{one_hot_encoding}_Dense{Dense_dim}_epoch{epoch+1}.h5')
    ...
    if auto_encoder == 'CVAE':
        ...
        _x_decoded_mean = decoder_mean(_h_decoded)
        decoder = Model(decoder_input, _x_decoded_mean)
        checkpointer = CustomModelCheckpoint(encoder, decoder, latent_dim, condition_dim, auto_encoder, dataset, save_interval=10)
        
        history = vae.fit([X_train, y_train], None, shuffle=True, epochs=epochs, batch_size=16, validation_data=([X_valid, y_valid], None), callbacks=[checkpointer])
...
def AE_load(X_test=None,
    ...
    # 重み付けとサンプル生成を行う場合
        ...
        if auto_encoder == 'CVAE' or auto_encoder == 'ICVAE2':
            if one_hot_encoding == True:
                latent_vector, _, _ = encoder.predict([X_test.reshape(1, len(X_test)), X_test_predict]) 
                # # 入力とクラスを結合
                ...
                latent_vector, _, _ = encoder.predict([X_test.reshape(1, len(X_test)), X_test_predict.reshape(1, -1)])
                
        # CVAE以外の場合
        elif auto_encoder == 'AE':
            latent_vector = encoder.predict(X_test.reshape(1, len(X_test)))
        else:
            latent_vector, _, _ = encoder.predict(X_test.reshape(1, len(X_test)))
        ...
        if auto_encoder == 'CVAE' or auto_encoder == 'ICVAE':
            ...
            samples = decoder.predict(z_cond) #202309051408修正
        # CVAE以外の場合
        else:
            samples = decoder.predict(latent_vectors)
